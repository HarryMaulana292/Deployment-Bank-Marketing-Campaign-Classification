...
housing_input = st.radio("Insert customer's housing credit", ["yes", "no"])
loan_input = st.radio("Insert customer's loan credit", ["yes", "no"])
contact_input = st.radio("Insert customer's last contact", ["cellular", "telephone", "other"])
with st.expander("Insert month of last contacted with customer"):
    month_input = st.radio(label = "Choose one", options = ["jan",
                                                        ...
                                                        "nov",
                                                        "dec"])
campaign_input = st.number_input("Insert how many contact performed with customer during this campaign",
                                 min_value = 0, step = 1)
pdays_input = st.number_input("Insert how many days after last contact with customer",
                                 min_value = -1, step = 1)
poutcome_input = st.radio("Insert the last campaign result", ["success", "failure",
                                                               "other", "unknown"])
